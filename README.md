...
| [释放扩散模型在端到端自动驾驶中的潜力](释放扩散模型在端到端自动驾驶中的潜力/) | 端到端自动驾驶 (扩散模型) | arXiv 2602.22801 | 26 |
...
| [面向生成建模的流匹配](面向生成建模的流匹配/) | 生成模型 (流匹配/扩散) | ICML 2023 (Outstanding Paper) | 28 |
| [离线强化学习中用于精确能量引导扩散采样的对比能量预测](离线强化学习中用于精确能量引导扩散采样的对比能量预测/) | 强化学习 (扩散模型/能量引导) | ICML 2023 | 31 |
| [SparseWorld：通过稀疏场景表示的世界模型增强端到端自动驾驶](SparseWorld：通过稀疏场景表示的世界模型增强端到端自动驾驶/) | 端到端自动驾驶 (世界模型) | arXiv 2605.24354 | 7 |
| [基于随机微分方程的基于分数的生成建模](基于随机微分方程的基于分数的生成建模/) | 生成模型 (SDE/扩散) | ICLR 2021 | 35 |
| [Flex](Flex/) | 端到端自动驾驶 (多相机编码/VLA) | arXiv 2512.10947 | 7 |
| [回归基础：让去噪生成模型去噪](回归基础：让去噪生成模型去噪/) | 扩散模型 (像素空间/Transformer) | arXiv 2511.13720 | 17 |
